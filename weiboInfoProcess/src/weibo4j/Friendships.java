...
public class Friendships extends Weibo {
	...
	public String[] getFriendsIdsByUid(String uid, int count) throws WeiboException {
		return User.constructIds(client.get(WeiboConfig.getValue("baseURL")
				+ "friendships/friends/ids.json",
				new PostParameter[] { new PostParameter("uid", uid) ,
	...
	public List<String> getAllFriendsIdsByUid(String uid, int count) throws WeiboException {
		int cursor = -1;
		List<String> allList = new ArrayList<String>();
		while(cursor != 0){
			Response flist = client.get(WeiboConfig.getValue("baseURL")
					+ "friendships/friends/ids.json",
					new PostParameter[] { new PostParameter("uid", uid) ,
				    new PostParameter("count", count), new PostParameter("cursor", cursor)},
					access_token);
			cursor = User.nextPage(flist);
			String[] friendsList = User.constructIds(flist);
			List<String> fl = Arrays.asList(friendsList);
			allList.addAll(fl);
		}
		return allList;
	}
	...
	public List<String> getAllFollowersIdsByUid(String uid, int count) throws WeiboException {
		int cursor = -1;
		List<String> allList = new ArrayList<String>();
		while(cursor != 0){
			Response flist = client.get(WeiboConfig.getValue("baseURL")
