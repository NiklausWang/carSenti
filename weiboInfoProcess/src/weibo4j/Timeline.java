...
public class Timeline extends Weibo {
	...
	public List<String> getAllUserTimelineIdsByUid(String uid, int timelineNum)
			throws WeiboException {
		int page = timelineNum / 100;                             //用户微博满100条的页数，比实际页数少一
		int lastNum = timelineNum - 100 * page;                   //用户微博最后一页微博数
		List<String> allStatusesIds = new LinkedList<String>();
		
		for(int i = 1; i <= page; i++){
			UserTimelineIds utemp = new UserTimelineIds(client.get(WeiboConfig.getValue("baseURL")
					+ "statuses/user_timeline/ids.json",
					new PostParameter[] { new PostParameter("uid", uid), 
				    new PostParameter("count", 100),
					new PostParameter("page", i)}, access_token));
			allStatusesIds.addAll(utemp.getStatusesIds());
		}
		
		page ++;
		if (lastNum > 0) {
			UserTimelineIds ltemp = new UserTimelineIds(client.get(
					WeiboConfig.getValue("baseURL")
							+ "statuses/user_timeline/ids.json",
					new PostParameter[] { new PostParameter("uid", uid),
							new PostParameter("count", lastNum),
							new PostParameter("page", page) }, access_token));
			allStatusesIds.addAll(ltemp.getStatusesIds());
		}
		return allStatusesIds;
	}
	...
	public JSONArray getReadCount(String ids) throws WeiboException {
		return client.get(
				WeiboConfig.getValue("baseURL") + "statuses/count_sp.json",
				new PostParameter[] { new PostParameter("ids", ids) },
				access_token).asJSONArray();
	}
}
